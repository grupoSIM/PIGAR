...
test("[notifications][postgres] dos workers, lease vencido y receptor inválido conservan estados seguros", async (t) => {
  ...
  const ids = [];
  t.after(async () => {
    await database.transactionalNotification.deleteMany({ where: { recipientProfileId: { in: [client.id, other.id] } } });
  ...
  assert.equal(await database.transactionalNotification.count({ where: { sourceEventId: event.id } }), 1);
  assert.equal((await database.outboxEvent.findUniqueOrThrow({ where: { id: event.id } })).state, "PROCESSED");
  ...
  ids.push(expired.id);
  await pollOnce();
  assert.equal((await database.outboxEvent.findUniqueOrThrow({ where: { id: expired.id } })).state, "PROCESSED");
  ...
  ids.push(invalid.id);
  await pollOnce();
  assert.equal((await database.outboxEvent.findUniqueOrThrow({ where: { id: invalid.id } })).state, "FAILED");
...
test("[notifications][postgres] la API pagina por propietario y conserva la primera lectura", async (t) => {
  ...
  t.after(async () => {
    await database.accessAuditEvent.deleteMany({ where: { actorProfileId: { in: [client.id, other.id] } } });
  ...
  for (let index = 0; index < 3; index += 1) {
    const source = await database.outboxEvent.create({
      data: { eventType: "work_order.en_route", version: 1, state: "PROCESSED", payload: { requestId: request.id } },
    });
  ...
  const [one, two] = await Promise.all([service.markRead(actor, first.items[0].id), service.markRead(actor, first.items[0].id)]);
...
test("[notifications][http] contrato devuelve 401, 403, 404 y errores seguros", async (t) => {
  ...
  const notice = await database.transactionalNotification.create({
    data: { sourceEventId: source.id, recipientProfileId: client.id, requestId: request.id, eventType: "WORK_ORDER_EN_ROUTE", templateKey: "technician-en-route", templateVersion: 1 },
  });
  ...
  assert.equal((await app.inject({ method: "GET", url: "/api/v1/notifications", headers: { "x-notification-test-actor": `ADMIN:${client.id}` } })).statusCode, 403);
  assert.equal((await app.inject({ method: "GET", url: "/api/v1/notifications?limit=51", headers: clientHeader })).statusCode, 400);
  assert.equal((await app.inject({ method: "GET", url: `/api/v1/notifications?cursor=${"x".repeat(513)}`, headers: clientHeader })).statusCode, 400);
  assert.equal((await app.inject({ method: "PUT", url: `/api/v1/notifications/${notice.id}/read`, headers: { "x-notification-test-actor": `CLIENT:${other.id}` } })).statusCode, 404);
  assert.equal((await app.inject({ method: "PUT", url: `/api/v1/notifications/${notice.id}/read`, headers: clientHeader })).statusCode, 200);
});
...
test("[notifications][postgres] asignación, reasignación, camino y cancelación emiten outbox sólo al transicionar", async (t) => {
  ...
  const operator = await database.profile.create({ data: { identitySubject: `notifications-operator-${randomUUID()}`, role: "ADMIN" } });
  const technicians = await Promise.all([
    database.technician.create({ data: { fullName: "Técnico A", phone: "+541100000001", status: "ACTIVE" } }),
    database.technician.create({ data: { fullName: "Técnico B", phone: "+541100000002", status: "ACTIVE" } }),
  ]);
  t.after(async () => {
    await database.$disconnect();
  });
  ...
  await orders.transition(actor, assigned.id, { action: OrderTransitionAction.REASSIGN_TECHNICIAN, expectedVersion: 1, technicianId: technicians[1].id, reason: "sintético" });
  await orders.transition(actor, assigned.id, { action: OrderTransitionAction.MARK_EN_ROUTE, expectedVersion: 2 });
  await orders.transition(actor, assigned.id, { action: OrderTransitionAction.CANCEL, expectedVersion: 3, reason: "sintético" });
  const events = await database.outboxEvent.findMany({ where: { aggregateId: assigned.id }, orderBy: { createdAt: "asc" } });
...
test("[notifications][postgres] conciliaciones concurrentes emiten un único pago rechazado", async (t) => {
  ...
  const charge = await database.charge.create({
    data: { workOrderId: order.id, categoryName: "Servicio", offerVersion: 1, currency: "ARS", amount: "1.00" },
  });
  const attempt = await database.paymentAttempt.create({
    data: { chargeId: charge.id, sequence: 1, externalReference: `payment-${randomUUID()}`, state: "PENDING" },
  });
  t.after(async () => {
    await database.$disconnect();
  });
  const billing = new BillingService(database, {
    ...
    getPayment: async () => { throw new Error("not used"); },
  ...
  assert.equal(await database.outboxEvent.count({ where: { aggregateId: order.id, eventType: "payment.rejected" } }), 1);
  assert.equal((await database.paymentAttempt.findUniqueOrThrow({ where: { id: attempt.id } })).state, "REJECTED");
});
