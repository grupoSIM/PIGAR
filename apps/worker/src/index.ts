...
const templates: Record<string, { type: string; key: string }> = {
  "work_order.assignment_changed": { type: "WORK_ORDER_ASSIGNMENT_CHANGED", key: "assignment-changed" },
...
export async function pollOnce(): Promise<void> {
  ...
  try {
    ...
    const claim = await client.query(`WITH candidate AS (SELECT id FROM outbox_event WHERE "eventType" = ANY($1) AND (state = 'PENDING' OR (state = 'PROCESSING' AND "leaseExpiresAt" < NOW())) AND "availableAt" <= NOW() ORDER BY "createdAt" FOR UPDATE SKIP LOCKED LIMIT 1) UPDATE outbox_event o SET state = 'PROCESSING', attempts = attempts + 1, "leaseExpiresAt" = NOW() + interval '30 seconds' FROM candidate WHERE o.id = candidate.id RETURNING o.id, o."eventType", o.version, o.payload, o.attempts, o."leaseExpiresAt"`, [Object.keys(templates)]);
    if (!claim.rowCount) { await client.query("COMMIT"); return logWorkerEvent("worker.poll.idle"); }
    const claimedEvent = claim.rows[0] as ClaimedEvent;
    event = claimedEvent;
    await client.query("COMMIT");
    const requestId = claimedEvent.payload?.requestId; const template = claimedEvent.version === 1 ? templates[claimedEvent.eventType] : undefined;
    if (!template || typeof requestId !== "string") return await fail(client, claimedEvent, "NOTIFICATION_EVENT_INVALID");
    const recipient = await client.query(`SELECT r."clientProfileId" FROM service_request r JOIN profile p ON p.id = r."clientProfileId" WHERE r.id = $1 AND p.role = 'CLIENT'`, [requestId]);
    if (!recipient.rowCount) return await fail(client, claimedEvent, "NOTIFICATION_RECIPIENT_INVALID");
    await client.query("BEGIN");
    const inserted = await client.query(`INSERT INTO transactional_notification ("recipientProfileId", "sourceEventId", "requestId", "eventType", "templateKey", "templateVersion") VALUES ($1, $2, $3, $4, $5, 1) ON CONFLICT ("sourceEventId", "recipientProfileId") DO NOTHING`, [recipient.rows[0].clientProfileId, claimedEvent.id, requestId, template.type, template.key]);
    const completed = await client.query(`UPDATE outbox_event SET state = 'PROCESSED', "processedAt" = NOW(), "leaseExpiresAt" = NULL WHERE id = $1 AND state = 'PROCESSING' AND "leaseExpiresAt" = $2`, [claimedEvent.id, claimedEvent.leaseExpiresAt]);
    await client.query("COMMIT");
    ...
    else logger.warn("notification.retry", undefined, { code: "NOTIFICATION_MATERIALIZATION_RETRY", duration_ms: 0 });
  }
...
async function fail(client: import("pg").PoolClient, event: ClaimedEvent, code: string) {
  await client.query(`UPDATE outbox_event SET state = 'FAILED', "leaseExpiresAt" = NULL WHERE id = $1 AND state = 'PROCESSING' AND "leaseExpiresAt" = $2`, [event.id, event.leaseExpiresAt]);
...
async function retry(client: import("pg").PoolClient, event: ClaimedEvent) {
  ...
  await client.query(`UPDATE outbox_event SET state = 'PENDING', "availableAt" = NOW() + ($2 * interval '1 second'), "leaseExpiresAt" = NULL WHERE id = $1 AND state = 'PROCESSING' AND "leaseExpiresAt" = $3`, [event.id, delaySeconds, event.leaseExpiresAt]);
  logger.warn("notification.retry", undefined, { code: "NOTIFICATION_MATERIALIZATION_RETRY", duration_ms: 0 });
}
